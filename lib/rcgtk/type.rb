...
module RCGTK
	...
	class NumberType < Type
		...
		def self.value_class
			begin
				@value_class ||=
				RCGTK.const_get(self.name.match(/::(.+)Type$/).captures.last.to_sym)
	...
	NativeIntType = RCGTK.const_get("Int#{FFI.type_size(:int) * 8}Type")
...
# {RCGTK::Type Type} or an instance of a sub-class of Type.  If a class is
...
#   *type* isn't a sub-class of {RCGTK::Type Type}, or if the type check
#   fails.
#
# @return [Type]  The object *o* or an instance of the class passed in parameter *o*.
def check_cg_type(o, type = RCGTK::Type, blame = 'type', strict = false)
	if o.is_a?(Class)
...
# sub-classses of {RCGTK::Type Type} or instances of a sub-class of Type.
...
def check_cg_array_type(array, type = RCGTK::Type, blame = 'el_types', strict = false)
	array.map do |o|
		if o.is_a?(Class)
			type_ok = if strict then o == type else o.subclass_of?(type) end
